...
Recorder сохраняет любой failure-result:
...
Для `timeout` и инфраструктурных ошибок `.args` особенно важны: они позволяют
повторить тот же набор аргументов и понять, почему worker не вернул обычный
результат.
...
- Recorder пишет `.args` для всех non-ok isolated results, а в in-process mode
  сохраняет перехваченные exceptions.
